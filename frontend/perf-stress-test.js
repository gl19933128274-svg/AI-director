    ...
    const req = http.request({
        ...
        res.on('end', () => {
            ...
            if (res.statusCode === 200 && data.includes('"success":true')) {
                metrics.success++;
                // 检查是否缓存命中（通过响应时间判断，<100ms认为是缓存命中）
                if (responseTime < 100) {
                    metrics.cacheHits++;
                }
            } else {
...
function printResults() {
    ...
    if (metrics.success > 0) {
        ...
        console.log(`  缓存加速请求: ${metrics.cacheHits}个 (响应<100ms)`);
    }
    ...
    console.log('  缓存加速: 无 -> ' + metrics.cacheHits + '次命中');
    console.log('========================================');
}

console.log('========================================');
console.log('  开始性能优化压力测试');
console.log('========================================');
