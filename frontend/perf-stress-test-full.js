/**
 * 全系统压力测试脚本
 * 测试配置：并发用户100，总请求1000，模拟AI高负载场景
 */

const http = require('http');

// 测试配置
const API_URL = 'http://localhost:3001/api/storyboard/generate';
    ...
    const req = http.request({
        ...
        res.on('end', () => {
            ...
            if (completedRequests % 50 === 0) {
                process.stdout.write(`\r进度: ${completedRequests}/${TOTAL_REQUESTS} (${((completedRequests/TOTAL_REQUESTS)*100).toFixed(1)}%)`);
            }
...
function printResults() {
    ...
    const timeoutRate = ((errorStats.timeout / TOTAL_REQUESTS) * 100).toFixed(2);
    const avgResponseTime = sortedTimes.length > 0 ? (sortedTimes.reduce((a, b) => a + b, 0) / sortedTimes.length).toFixed(2) : '0';

    console.log('\n');
    console.log('========================================');
    console.log('       全系统压力测试报告');
    console.log('========================================');
    ...
    console.log(`  失败请求: ${metrics.failed} (${(100 - parseFloat(successRate)).toFixed(2)}%)`);
    ...
    console.log('\n【超时率】');
    console.log(`  超时请求: ${errorStats.timeout} (${timeoutRate}%)`);
    ...
    if (totalErrors > 0) {
        ...
        for (const [type, count] of Object.entries(errorStats)) {
            if (count > 0) {
                console.log(`  ${type}: ${count}次 (${((count / totalErrors) * 100).toFixed(1)}%)`);
            }
        }
    }
    ...
    // 评估标准
    const isPass = parseFloat(successRate) >= 95 && parseFloat(timeoutRate) < 5;
    console.log(`  测试结果: ${isPass ? '✅ 通过' : '❌ 未通过'}`);
    console.log(`  评估标准:`);
    console.log(`    - 成功率 ≥ 95%: ${parseFloat(successRate) >= 95 ? '✅' : '❌'}`);
    console.log(`    - 超时率 < 5%: ${parseFloat(timeoutRate) < 5 ? '✅' : '❌'}`);
    console.log('========================================');
}

console.log('========================================');
console.log('     全系统压力测试开始');
console.log('========================================');
